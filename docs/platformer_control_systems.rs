...
pub fn apply_platformer_controls(
	...
	if egui_context.ctx_mut().wants_keyboard_input() {
		for (_, mut controller, ..) in query.iter_mut() {
			// The basis remembers its last frame status, so if we cannot feed it proper
			// input this frame (for example - because the GUI takes the input focus) we
	...
	{
		// This part is just keyboard input processing. In a real game this would
		// probably be done with a third party plugin.
		...
		let turn_in_place =
			forward_from_camera.is_none() && keyboard.any_pressed([KeyCode::AltLeft, KeyCode::AltRight]);
		...
		// This needs to be called once per frame. It lets the air actions counter know
		// about the air status of the character. Specifically:
		...
		// This also needs to be called once per frame. It checks which obstacles needs
		// to be blocked - e.g. because we've just finished an action on them and we
		// don't want to reinitiate that action.
		blip_reuse_avoidance.update(controller.as_ref(), obstacle_radar);

		// Here we will handle one-way platforms. It looks long and complex, but it's
		// actual several schemes with observable changes in behavior, and each
		...
		match config.falling_through {
			...
			FallingThroughControlScheme::JumpThroughOnly => {
				...
				// To achieve this, we simply take the first platform detected by the ghost
				// sensor, and treat it like a "real" platform.
			...
			// certain limitations:
			//
			// 1. If the player releases the crouch button before the character has passed a certain
			//    distance it'll climb back up on the platform.
			// 2. If a ghost platform is too close above another platform (either ghost or solid), such
			//    that when the character floats above the lower platform the higher platform is detected
			//    at above-minimal proximity, the character will climb up to the higher platform - even
			//    after explicitly dropping down from it to the lower one.
			//
			...
			FallingThroughControlScheme::WithoutHelper => {
				...
				// finds with a proximity higher than the defined minimum. We either treat it as
				// a real platform, or ignore it and any other platform the sensor has
				// found.
			...
			// through:
			//
			// * Pressing the crouch button while standing on a ghost platform will make the character
			//   fall through it.
			// * Even if the button is released immediately, the character will not climb back up. It'll
			//   continue the fall.
			// * Even if the button is held and there is another ghost platform below, the character will
			//   only drop one "layer" of ghost platforms.
			// * If the player drops from a ghost platform to a platform too close to it - the character
			//   will not climb back up. The player can still climb back up by jumping, of course.
		...
		// The basis is Tnua's most fundamental control command, governing over the
		// character's regular movement. The basis (and, to some extent, the actions
		// as well) contains both configuration - which in this case we copy over from
		// `config.walk` - and controls like `desired_velocity` or `desired_forward`
		// which we compute here based on the current frame's input.
				...
				if let Some((climbable_entity, action)) = already_climbing_on.as_ref() {
						...
						.unwrap()
					{
						Ordering::Less => {
							if controller.is_airborne().unwrap() {
								...
								if extent < 0.9 * LOOK_ABOVE_OR_BELOW {
									action.hard_stop_down = Some(blip.closest_point().get() - extent * Vector3::Y);
								}
							} else {
								if initiation_direction == Vector3::ZERO {
									continue 'blips_loop;
						...
						Ordering::Greater => {
							let extent = blip.probe_extent_from_closest_point(Dir3::Y, LOOK_ABOVE_OR_BELOW);
							if extent < 0.9 * LOOK_ABOVE_OR_BELOW {
								action.hard_stop_up = Some(blip.closest_point().get() + extent * Vector3::Y);
							}
						}
					}

					controller.action(action);
				} else if let TnuaBlipSpatialRelation::Aeside(blip_direction) = blip.spatial_relation(0.5) {
				...
				TnuaBlipSpatialRelation::Aeside(blip_direction) => {
					...
					if controller.is_airborne().unwrap() {
						...
						if dot_direction <= -0.7 {
							if let Some((best_entity, best_dot, best_direction)) = walljump_candidate.as_mut() {
								...
								walljump_candidate = Some((blip.entity(), dot_direction, blip_direction));
							}
						}
						if dot_threshold < dot_direction && 0.8 < blip.flat_wall_score(Dir3::Y, &[-1.0, 1.0]) {
		...
		if crouch {
			// Crouching is an action. We either feed it or we don't - other than that there
			// is nothing to set from the current frame's input. We do pass it through
			// the crouch enforcer though, which makes sure the character does not stand
			// up if below an obstacle.
...
/// Since the fixed timestep schedule does not cache just pressed states that
/// happened in a frame with no fixed updates, we need to cache them ourselves
/// in order to not miss them. Note that if you use a smarter input manager like
/// LWIM, this is handled for you. If the demo is running with a variable
/// timestep, this will just report the current frame's state as expected.
