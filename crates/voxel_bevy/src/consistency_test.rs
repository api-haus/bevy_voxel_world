...
//!
//! The stress test deliberately exposes a race condition that causes "ghost cells":
//!
//! 1. Refinement 1 runs: subdivides to LOD N, starts async mesh generation
//! 2. Refinement 2 runs (camera moved): merges back to LOD N+1, updates world.leaves
...
impl ConsistencyValidator {
	...
	fn on_pending_started(&mut self, nodes_to_mesh: &[OctreeNode]) {
		...
		self.stats.max_concurrent_pending = self.stats.max_concurrent_pending.max(self.pending_nodes.len());
	}
	...
	/// Note: Some nodes may be legitimately empty (homogeneous), so this is informational.
			...
			if !self.entity_spawned_frame.contains_key(node) && !self.pending_nodes.contains(node) {
				if let Some(&added_frame) = self.node_added_frame.get(node) {
					...
					if age > MISSING_THRESHOLD_FRAMES {
						// This could be a legitimate empty chunk, so don't record as violation
						// Just track it for debugging if needed.
					}
				}
			}
		}
...
/// Simulated async refinement state (mirrors AsyncRefinementState in noise_lod.rs).
...
fn poll_refinement_system(
	...
	for node in &spawned_nodes {
		...
		let entity = commands.spawn(VoxelChunk {
			node: *node,
...
mod tests {
	...
	///
	/// This test is EXPECTED to find violations until the ghost cell bug is fixed.
	/// It runs multiple passes to increase the chance of hitting the race condition.
	///
