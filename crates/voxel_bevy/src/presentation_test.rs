...
//!
//! Tests the exact logic from noise_lod's poll_async_refinement without Bevy runtime.
//! Simulates entity spawn/despawn tracking to detect orphans.
...
use voxel_plugin::octree::{refine, OctreeConfig, OctreeNode, RefinementBudget, RefinementInput, TransitionGroup};
...
impl MockBevyPresentation {
    ...
    /// 2. Spawn new chunks from ready_chunks (WITH guard: only if still in leaves)
...
fn test_bevy_presentation_consistency() {
    ...
    for frame in 0..NUM_FRAMES {
        if start.elapsed() > MAX_TEST_DURATION {
            println!("Test timeout after {} frames ({:?})", frame, start.elapsed());
            break;
        }
...
fn test_bevy_presentation_rapid_zoom() {
    ...
    for cycle in 0..50 {
        if start.elapsed() > MAX_TEST_DURATION {
            println!("Test timeout after {} cycles ({:?})", cycle, start.elapsed());
            break;
        }
...
/// 1. Refinement 1: subdivide, update leaves, capture transitions to process "async"
...
fn test_ghost_cells_with_async_gap() {
    ...
    println!("Initial state: {} leaves, {} entities", world_leaves.len(), presentation.entity_count());
    ...
    println!("Subdivide produced {} ready chunks", subdivide_ready_nodes.len());

    // === STEP 2: BEFORE spawning, zoom back out (merge) ===
    // This simulates: async is in-flight, but camera moved, triggering new refinement
    ...
    println!("Stale nodes (would be ghosts without guard): {}", stale_count);
    ...
    println!("Final leaves: {}, entities: {}", world_leaves.len(), presentation.entity_count());
    ...
    assert!(
        ...
        "Expected ghost cells from async timing gap. Stale count was {}. \
        This test demonstrates why the spawn guard is necessary.",
        stale_count
    );
}
