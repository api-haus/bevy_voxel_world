...
impl VolumeSampler for TerrainWithCavesTestSampler {
  fn sample_volume(
    ...
    // Domain warp
    let warp_x =
      NoiseBuilder::fbm_3d_offset(start_x / vs, SIZE, start_y / vs, SIZE, start_z / vs, SIZE)
    ...
    for idx in 0..SAMPLE_SIZE_CB {
      ...
      let warp_magnitude =
        (warp_x[sn3d_idx].powi(2) + warp_y[sn3d_idx].powi(2) + warp_z[sn3d_idx].powi(2)).sqrt();
...
/// Mock presentation layer that tracks which nodes are "visible" (have
/// entities).
...
fn test_async_timing_gap_simulation() {
  ...
  println!(
    "After start: world.leaves={}, presented={}",
    world_leaves.len(),
    presentation.presented.len()
  );

  // Frame 1-4: Simulate frames passing
  // During this gap, presentation IS inconsistent with world.leaves - this is
  // EXPECTED for async The old parent is still "presented" but was removed from
  // world.leaves We do NOT check for orphans here - that would be a false
  // positive
  ...
  println!(
    "After poll: world.leaves={}, presented={}",
    world_leaves.len(),
    presentation.presented.len()
  );

  // NOW check for orphans - AFTER poll has completed, consistency should be
  // restored
...
/// Test that verifies the EXACT sequence of nodes_to_mesh matches what gets
/// spawned.
#[test]
fn test_nodes_to_mesh_matches_ready_chunks() {
  ...
  // Process
  let ready_chunks = process_transitions(world_id, &[transition], &sampler, &leaves, &config);
...
fn test_end_to_end_noise_lod_scene() {
  ...
  // Camera path that stays within world bounds (-500 to 500 in XZ, -100 to 100 in
  // Y)
