...
pub(crate) fn apply_remeshes(
	...
	for ev in evr.read() {
		...
		if meshes_vec.is_empty() {
			// trace!(target: "vox", "apply_mesh: empty meshes_vec for entity {:?}",
			// ev.entity);
			commands
				.entity(ev.entity)
				.remove::<super::NeedsInitialMesh>();
		...
		// info!(target: "vox", "apply_mesh: mesh bounds {:?} for entity {:?}", aabb,
		// ev.entity);
		...
		if let Ok((chunk, _storage, mut transform)) = q_chunk_tf.get_mut(ev.entity) {
			...
			// info!(target: "vox", "apply_mesh: adding mesh to entity {:?} at position
			// {:?}", ev.entity, transform.translation);
