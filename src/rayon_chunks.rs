use fast_surface_nets::glam::Vec3A as FSNVec3A;
use fast_surface_nets::ndshape::{ConstShape, ConstShape3u32};
use fast_surface_nets::{SurfaceNetsBuffer, surface_nets};
use ilattice::prelude::Vec3A as ILVec3A;
...
///
/// This function is behind the `obj_export` feature to avoid introducing an extra dependency
/// unless specifically requested.
#[cfg(feature = "obj_export")]
pub fn write_mesh_to_obj_file(
    name: String,
    buffers: &[(FSNVec3A, SurfaceNetsBuffer)],
) -> std::io::Result<()> {
...
fn generate_and_process_chunk(
    ...
    spheres: &[Sphere],
) -> Option<(FSNVec3A, SurfaceNetsBuffer)> {
    ...
    padded_chunk_extent.iter3().for_each(|pwo| {
        ...
        for sphere in intersecting_spheres.iter() {
            ...
            // Compute distance without mixing Vec3A types from different crates
            let dx = sphere.origin.x - pwof.x;
            let dy = sphere.origin.y - pwof.y;
            let dz = sphere.origin.z - pwof.z;
            let dist = (dx * dx + dy * dy + dz * dz).sqrt();
            *v = v.min(dist - sphere.radius);
        }
    ...
    if some_pos_found && some_neg_or_zero_found {
        ...
        if sn_buffer.positions.is_empty() {
            ...
            let min = padded_chunk_extent.minimum.as_vec3a();
            Some((FSNVec3A::new(min.x, min.y, min.z), sn_buffer))
        }
    } else {
...
pub fn generate_chunks() -> Vec<(FSNVec3A, SurfaceNetsBuffer)> {
    ...
    let spheres: Vec<Sphere> = (0..501)
        .map(|_| {
            let origin = FSNVec3A::new(
                rng.gen_range(-74.0..74.0),
                rng.gen_range(-74.0..74.0),
                rng.gen_range(-74.0..74.0),
            );
            let radius = rng.gen_range(2.5..5.0);
            // Convert to ilattice's Vec3A for extent math
            let origin_il = ILVec3A::new(origin.x, origin.y, origin.z);
            let extent = Extent::from_min_and_lub(origin_il, origin_il)
                .padded(radius)
